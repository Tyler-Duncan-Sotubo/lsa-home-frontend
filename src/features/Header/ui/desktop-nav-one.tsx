          ...
          if (item.type === "mega") {
            ...
            return (
              <button
                key={item.label}
                type="button"
                onMouseEnter={() => setOpenLabel(item.label)} // ✅ open on hover
                onFocus={() => setOpenLabel(item.label)} // ✅ keyboard support
                onClick={() => setOpenLabel(isOpen ? null : item.label)} // optional click toggle
                className={[
