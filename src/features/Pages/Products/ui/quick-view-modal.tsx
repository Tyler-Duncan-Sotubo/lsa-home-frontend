...
import { QuickViewDialogProps } from "@/features/Pages/Products/types/products";
import { ProductGallery } from "./product-gallery";
import { ProductDetailsOne } from "./product-details-one";
...
export function QuickViewDialog({
  ...
  const { quickViewEnabled, quickViewVariant } = useAppSelector(
    (s) => s.runtimeConfig.features
  );
  ...
  if (!quickViewEnabled) return null;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        {loading && (
          <Skeleton className="absolute inset-0 h-full w-full bg-white/70" />
        )}
      </DialogContent>
    </Dialog>
  );
